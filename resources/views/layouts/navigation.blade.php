<nav x-data="{ open: false }" class="bg-slate-900 border-slate-800 text-slate-100 border-b border-slate-800">
            ...
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('dashboard') }}">
                        <img src="{{ asset('images/Logo.png') }}" alt="Logo" class="h-9 w-auto fill-current text-slate-200">
                    </a>
                </div>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                @auth
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-slate-400 bg-slate-900 border-slate-800 text-slate-100 hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                ...
                @else
                    <div class="space-x-4">
                        ...
                        <a href="{{ route('register') }}" class="rounded-lg bg-cyan-600 px-4 py-2 text-sm font-medium hover:bg-cyan-500 text-white shadow-lg shadow-cyan-900/20 transition">Criar conta</a>
                    </div>
                @endauth
            </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <!-- Responsive User Info and Authentication -->
        @auth
        ...
        @else
            <div class="pt-4 pb-1 border-t border-slate-800">
                <div class="flex flex-col space-y-2 px-4">
                    <a href="{{ route('login') }}" class="block px-3 py-2 text-base font-medium text-slate-300 hover:text-white hover:bg-slate-800 rounded-md">Entrar</a>
                    <a href="{{ route('register') }}" class="block px-3 py-2 text-base font-medium text-white bg-cyan-600 hover:bg-cyan-500 rounded-md">Criar conta</a>
                </div>
            </div>
        @endauth
    </div>
